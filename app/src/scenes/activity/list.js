...
const Activities = ({ date, user, project }) => {
  ...
  function onUpdateValue(i, j, value, date) {
    const n = [...activities];
    n[i].detail[j].value = value;
    n[i].detail[j].date = date;
                        ...
                        <tr className="border-t border-b border-r border-[#E5EAEF]" key={`1-${e._id}`} onClick={() => setOpen(i)}>
                          <th className="w-[100px] border-t border-b border-r text-[12px] font-bold text-[#212325] text-left">
                            <div className="flex flex-1 items-center justify-between gap-1 px-2">
                              ...
                              <div className="flex flex-col items-end">
                                ...
                                {e.total && <div className="text-[10px] italic font-normal">{(((e.total / 8).toFixed(2) / getTotal()) * 100).toFixed(2)}%</div>}
                              </div>
                            </div>
                          </th>
                          {e.detail.map((f, j) => {
                            return (
                              <Field
                                ...
                                invoiced={e.invoiced}
                                value={f.value || 0}
                                onChange={(a) => onUpdateValue(i, j, parseFloat(a.target.value || 0), new Date(date.getFullYear(), date.getMonth(), j + 1))}
                              />
                            );
                          })}
                          <th className={`border border-[#E5EAEF] py-[6px]`}>
                            <div
                              className={`flex justify-center cursor-pointer text-xl hover:text-red-500 rounded-full text-white bg-red-500 hover:bg-red-300 transition-colors py-2 px-3 mx-3`}>
                          ...
                          <th className={`border border-[#E5EAEF] py-[6px]`}>
                            <div className={`flex justify-center cursor-pointer text-xl hover:text-red-500`}>
                              <LoadingButton
                                className="bg-[#009dff] hover:bg-[#0069d9] text-[#fff] rounded-full m-auto block text-sm p-2"
                                onClick={() => history.push(`/activity/${e._id}`)}
                                color="primary">
                                View details
                              </LoadingButton>
                            </div>
